...
class CosaArgGroup(argparse._ArgumentGroup):
    ...
    def add_argument(self, *args, default=None, dest:str=None, is_config_file:bool=False, **kwargs):
        option_name = self._add_long_option(self._category, args)
        if is_config_file:
...
class CosaArgParser(argparse.ArgumentParser):
    ...
    def add_argument(self, *args, default=None, dest:str=None, is_config_file:bool=False, **kwargs):
        ...
        option_name = self._add_long_option(BUILTIN, args)
        if dest is None:
            dest = option_name
        if is_config_file:
            self._config_files.add(option_name)
    ...
    def _add_long_option(self, namespace:str, options:Sequence[str])->str:
        '''
...
def test():
    ...
    in_options = parser.add_general_group('input').add_mutually_exclusive_group()

    in_options.set_defaults(input_files=None)
    in_options.add_argument('--input_files', '-i', metavar='<input files>', type=str, required=False)
    ...
    # this is just set internally automatically
    ...
    verification_options = parser.add_problem_group('analysis').add_mutually_exclusive_group()

    verification_options.set_defaults(verification=None)
    verification_options.add_argument('--verification', type=str,
                                      choices=['safety', 'ltl', 'fsm-check', 'parametric'])

    verification_options.add_argument('--equivalence', metavar='<input files>', type=str, required=False,
                                  help='equivalence checking using BMC.')

    verification_options.set_defaults(simulate=False)
    verification_options.add_argument('--simulate', dest='simulate', action='store_true',
                       help='simulate system using BMC.')

    problem_type = parser.get_problem_type()
